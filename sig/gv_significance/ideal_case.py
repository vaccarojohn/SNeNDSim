...
import numpy as np
from gv_significance.significance_from_pvalue import significance_from_pvalue
...
def five_sigma_threshold(B, detection_efficiency):
    ...
    assert detection_efficiency in legal_efficiencies, \
        "Detection efficiency must be one of %s" % ",".join(str(s) for s in legal_efficiencies)
