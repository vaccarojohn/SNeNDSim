...
if __name__ == "__main__":
    ...
    for i in range(200):
        ...
        for seg in f['segments']:
            if seg['event_id'] > event_id:
                # Save only events with energy deposition in the TPC so that data agrees with data from cosmics_segment_data.py
                if temp_inDet:
    ...
    np.savez_compressed(outfile_dir + '/cosmics_selection_data.npz', pmaxe=data_pmaxe, crt=data_crt, light=data_light, tmin=data_tmin, 
                        tmax=data_tmax, trms=data_trms, tseg=data_tseg, tdiff=data_tdiff)
